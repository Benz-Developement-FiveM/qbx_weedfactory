...
Use `accountPrefix = 'society_'` only if your Renewed-Banking account is named like `society_whitewidow`.

Make sure `Renewed-Banking` starts before `qbx_weedfactory` in `server.cfg`.
...
You can also add `category = 'category_key'` directly inside any rollable, edible, bag, or strain config entry to force it into a specific menu category.
